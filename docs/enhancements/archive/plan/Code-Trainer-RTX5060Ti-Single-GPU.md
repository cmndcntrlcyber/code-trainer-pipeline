# Code-Trainer: RTX 5060 Ti Single-GPU Architecture
...
Code-Trainer is optimized for a single RTX 5060 Ti 16GB, leveraging Blackwell's Tensor Cores for efficient training and inference while offloading heavy Qwen-14B fine-tuning to HuggingFace Skills cloud.
...
```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│                    CODE-TRAINER: RTX 5060 Ti SINGLE-GPU PIPELINE                 │
├─────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                      │
│  ╔════════════════════════════════════════════════════════════════════════════════╗ │
...
class RTX5060TiTrainingConfig:
    ...
    # Wandb
    wandb_project: str = "code-trainer"
...
preprocessing:
  dataset_name: ${HF_USERNAME}/code-trainer-dataset
...
python -m phase5_deployment.scripts.convert_to_gguf \
    --model ${HF_USERNAME}/qwen14b-code-trainer-standard \
    --quantization Q4_K_M

# Start Inference Server
llama-server \
    -hf ${HF_USERNAME}/qwen14b-code-trainer-gguf:Q4_K_M \
