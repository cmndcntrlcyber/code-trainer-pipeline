"""
phase5_gemma_deployment/gguf/converter.py

GGUF conversion pipeline for Gemma-4-12B: merge LoRA → convert to GGUF F16 → quantize.

Steps:
    1. Download fine-tuned LoRA adapter from HF Hub
    2. Merge LoRA with base google/gemma-4-12B-it (CPU, float16)
    3. Convert merged model to GGUF F16
    4. Quantize to Q4_K_M (~7.5 GB)
"""
import logging
import shutil
import subprocess
...
class GGUFConverter:
    ...
    def download_base_model(
        ...
        if local_dir.exists() and any(local_dir.glob("*.safetensors")):
            ...
            return local_dir
        logger.info(f"Downloading base model: {model_id} (large download ~24GB)")
        snapshot_download(
            repo_id=model_id,
