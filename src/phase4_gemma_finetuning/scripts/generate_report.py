"""
phase4_gemma_finetuning/scripts/generate_report.py

Aggregate Phase 4A Gemma sweep results: download phase4-result.json from each
config's adapter repo on the Hub, rank by eval_loss, write a summary table.

Usage:
    set -a && source .env && set +a
    python -m src.phase4_gemma_finetuning.scripts.generate_report

    # Custom adapter base / output dir:
    python -m src.phase4_gemma_finetuning.scripts.generate_report \
        --adapter-base cmndcntrlcyber/gemma4-12b-code-trainer \
        --out docs/sweep/phase4a-gemma-summary.md
"""
import argparse
import json
import logging
...
from src.phase4_gemma_finetuning.configs.sweep_configs import SWEEP_CONFIGS
...
def main():
    parser = argparse.ArgumentParser(description="Aggregate Phase 4A Gemma sweep results")
    parser.add_argument("--config", default="src/config/config.yaml")
    parser.add_argument("--adapter-base", default=None)
    parser.add_argument("--out", default="docs/sweep/phase4a-gemma-summary.md")
    ...
    adapter_base = (
        ...
        or gf_cfg.get("output_base")
    )
    if not adapter_base:
        raise SystemExit("adapter base not set; pass --adapter-base or configure gemma_finetuning.cloud.adapter_base")

    token = os.environ.get("HF_TOKEN") or os.environ.get("HUGGINGFACE_TOKEN")
    if not token:
        raise SystemExit("HF_TOKEN required to read result files from the adapter repo")

    job_ids_path = Path(args.results_dir) / "gemma_job_ids.json"
    ...
    lines = []
    lines.append("# Phase 4A Gemma — Validation Sweep Summary")
    lines.append("")
    lines.append(f"**Adapter base:** `{adapter_base}-{{standard,aggressive}}`")
    lines.append("")
    ...
    if completed:
        ...
        lines.append("")
        lines.append("```bash")
        lines.append("python -m src.phase4_gemma_finetuning.scripts.launch_full_training \\")
